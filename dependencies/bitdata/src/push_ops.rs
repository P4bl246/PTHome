...
use crate::align_ops::*;
use crate::bit_ops::*;
use crate::data_size_ops::*;
use crate::raw_ptr_ops::{get_as_be_bytes, get_raw};
use crate::traits::{SizeOfT, PushOptimized, ImplWrite, ImplPush, And, Or, UnconstrainedAdd};
          ...
          match *write_strategy{
            BytesSlice::_8=>{
              ...
              if rest > 0{
                ...
                let byte = *get_as_be_bytes(raw, sz, 0).unwrap_or(&0x00u8);
                byte.write(slf, &vars2, write_strategy)?;
              }
              return Ok(());
            },
            BytesSlice::_16=>{
              ...
              if rest > 0{
                ...
                let byte = u16::from_be_bytes([*get_as_be_bytes(raw, sz, 0).unwrap_or(&0x00), *get_as_be_bytes(raw, sz, 1).unwrap_or(&0x00)]);
                byte.write(slf, &vars2, write_strategy)?;
              }
              return Ok(());
            },
            BytesSlice::_32=>{
              ...
              for mut _i in 0..n {
                let byte = u32::from_be_bytes([*raw.unc_add(_i)?, *get_raw(raw, sz, sum(_i, 1)?).unwrap_or(&0x00), *get_raw(raw, sz, sum(_i, 2)?).unwrap_or(&0x00),
                 *get_raw(raw, sz, sum(_i, 3)?).unwrap_or(&0x00)]);
                byte.write(slf, &vars2, write_strategy)?;
                _i=sum(_i,3)?;
              }
              let rest = sub(vars.size, bytes_to_bits(n)?);
              if rest > 0{
                ...
                let byte = u32::from_be_bytes([*raw.unc_add(sz)?, *get_as_be_bytes(raw, sz, 1).unwrap_or(&0x00),
                *get_as_be_bytes(raw, sz, 2).unwrap_or(&0x00),*get_as_be_bytes(raw, sz, 3).unwrap_or(&0x00)]);
            ...
            BytesSlice::_64=>{
              ...
              for mut _i in 0..=n {
                let byte = u64::from_be_bytes([*raw.unc_add(_i)?,*get_raw(raw, sz, sum(_i, 1)?).unwrap_or(&0x00),*get_raw(raw, sz, sum(_i, 2)?).unwrap_or(&0x00),
              ...
              if rest > 0{
                ...
                let byte = u64::from_be_bytes([*raw.unc_add(sz)?, *get_as_be_bytes(raw, sz, 1).unwrap_or(&0x00),  *get_as_be_bytes(raw, sz, 2).unwrap_or(&0x00),
                 *get_as_be_bytes(raw, sz, 3).unwrap_or(&0x00),  *get_as_be_bytes(raw, sz, 4).unwrap_or(&0x00), 
                  *get_as_be_bytes(raw, sz, 5).unwrap_or(&0x00),  *get_as_be_bytes(raw, sz, 6).unwrap_or(&0x00),  *get_as_be_bytes(raw, sz, 7).unwrap_or(&0x00)]);
    ...
    $(
      impl ImplPush for $ty where $ty:ImplWrite{
        ...
        fn push (&self, slf:&mut DataB, vars:&mut Vars)->Result<(),AllocErr>{
          let s = sel_write_strategy(slf.get_vec(),slf.get_size(),  vars.offset, vars.t_size, vars.remaining_space)?;
            match s.0{
              None =>{},
...
macro_rules! derive_pushoptimized{
  ...
  $( 
    ...
    $ty:ImplPush + SizeOfT{
      ...
      fn push_at_buffer(&self, slf:&mut DataB, bit_index:usize, bit_size:&mut Option<usize>, manage_overflow:Option<OverFlow>)->Result<(), AllocErr>{
        let mut size = bytes_to_bits(std::mem::size_of_val(self) )?;
        if let Some(v) = bit_size{
          if check_num(*v, bytes_to_bits(std::mem::size_of_val(self) )?){*v=bytes_to_bits(std::mem::size_of_val(self) )?}
            size=*v;
            }
             ...
             match size{
            0..=8=>{ 
              return u8::from_be_bytes([*arr.unc_add(0)?]).push_at_buffer(slf, bit_index, bit_size, manage_overflow);
            }
            9..=16=>{
              return u16::from_be_bytes([*arr.unc_add(0)?, *arr.unc_add(1)?]).push_at_buffer(slf, bit_index, bit_size, manage_overflow);
            },
            17..=32=>{
              return u32::from_be_bytes([*arr.unc_add(0)?, *arr.unc_add(1)?,
              *arr.unc_add(2)?, *arr.unc_add(3)?]).push_at_buffer(slf, bit_index, bit_size, manage_overflow);
            },
            33..=64=>{
              return u64::from_be_bytes([*arr.unc_add(0)?, *arr.unc_add(1)?, *arr.unc_add(2)?,
              *arr.unc_add(3)?, *arr.unc_add(4)?,
              *arr.unc_add(5)?,*arr.unc_add(6)?, *arr.unc_add(7)?]).push_at_buffer(slf, bit_index, bit_size, manage_overflow);
            },
             _=>{
  ...
  fn push_at_buffer(&self, slf:&mut DataB, bit_index:usize, bit_size:&mut Option<usize>, manage_overflow:Option<OverFlow>)->Result<(), AllocErr> {
    if let Some(bt) = bit_size{
      if *bt == 1{
        ...
        if check_num(sub(b.0,1),slf.get_size() ){return Err(AllocErr::OutOfBounds(slf.get_size() ));}
        let mask = shift(0x80u8, b.1.unwrap_or(0) as u8, true);
        
        move_ptr_val(slf, bytes_to_bits(b.0-1)?, b.1.unwrap_or(0))?;
      
        unsafe{
        let value = (mask & *self) | (!mask & *(slf.get_vec().unc_add(b.0 )?));
 ...
 impl ImplWrite for u8{
  #[inline(always)]
   fn write<T>(&self, slf:&mut DataB, vars:&Vars, _write_strategy:&BytesSlice<T>)->Result<(),AllocErr>{
    unsafe{ 
    let ptr = slf.get_vec().unc_add(vars.offset)?;
    if !vars.no_zero_bit || !vars.byte_overflow{
      ...
      if vars.no_bit_size{    
        ...
        _j+=put_datas(ptr, vars.t_size ,&_masks, &_d, _j)?;
  ...
  fn write<T>(&self, slf:&mut DataB, vars:&Vars, write_strategy:&BytesSlice<T>)->Result<(),AllocErr>{
    ...
    if !vars.no_zero_bit || !vars.byte_overflow{
      ...
      if vars.no_bit_size{    
        ...
        match ptr_l{
          TypedPtr::U8(raw)=>{
            let ptr = raw.unc_add(vars.offset)?;
 ...
 impl ImplWrite for u32{
  ...
  fn write<T>(&self, slf:&mut DataB, vars:&Vars, write_strategy:&BytesSlice<T>)->Result<(),AllocErr>{
    ...
    if !vars.no_zero_bit || !vars.byte_overflow{
      ...
      let shft = mult(BYTE_LENGTH,4)? as u64-bit ;
  ...
  fn write<T>(&self, slf:&mut DataB, vars:&Vars, write_strategy:&BytesSlice<T>)->Result<(),AllocErr>{
    ...
    if !vars.no_zero_bit || !vars.byte_overflow{
      if vars.no_bit_size{
        ...
        match ptr_l{
          TypedPtr::U8(i)=>{
            ...
            let last = sum(vars.offset, 7)?;
              let data = and(*self, shift(0xFFFFFFFFFFFFFFFF, 64-shft as u64,false)).to_be_bytes();
               ...
               copy_data(i, vars.offset+4, merge(slf.get_vec(), vars.offset+4, data[4], mask, sub(s, mult(BYTE_LENGTH, 3)? as u8), mult(sub(s as usize, mult(BYTE_LENGTH, 3)?),2)? as u8));
              ...
              Vars::fits(slf, merge(slf.get_vec(), last, data[7], mask, sub(sub(s , mult(BYTE_LENGTH, 3)? as u8), mult(BYTE_LENGTH, 2)? as u8)as u8, BYTE_LENGTH as u8), vars.size, last)?;
          },
          TypedPtr::U16(i)=>{
            let mask = 0xFFFFu16;
            let shft = vars.size as u16;
            ...
            let last = sum(offset, 3)?;
              ...
              Vars::fits(slf, merge(i, vars.offset, data[3], mask, sub(mult(BYTE_LENGTH, 2)? as u16, s), mult(BYTE_LENGTH, 2)? as u16), vars.size, last)?;
          },
          TypedPtr::U32(i)=>{
            let mask = 0xFFFFFFFFu32;
          ...
          TypedPtr::U64(i)=>{
            ...
            let offset = div(sum(vars.offset,7)?, 8)?;
            let shift = vars.size ;
            Vars::fits(slf, merge::<u64>(i, offset, *self, mask, shift as u64, mult(BYTE_LENGTH,8)? as u64), vars.size, offset)?;
          },
          _=>{return Err(AllocErr::UnrecognizedInstruction);}
        }  
      }
      ...
      if vars.no_bit_size{    
        ...
        let masks =  shift(mask, 64-bit, false) | shift(mask,64-(BYTE_LENGTH as u64-shft), true); 
        match ptr_l{
          TypedPtr::U8(raw)=>{
            let ptr = raw.unc_add(vars.offset)?;
            let d_temp = data.to_be_bytes();
            let mask_v_temp = masks.to_be_bytes();
            let carry = shift(*self, BYTE_LENGTH as u64-bit, false) as u8;
          ...
          TypedPtr::U16(raw)=>{
            ...
            let d = data.to_be_bytes();
            let mask_v = masks.to_be_bytes();
            let carry = shift(shift(*self & 0xFF, BYTE_LENGTH as u64-bit, false),BYTE_LENGTH as u64, false) as u16;
            let m = [u16::from_be_bytes([mask_v[0], mask_v[1]]),u16::from_be_bytes([mask_v[2], mask_v[3]]), 
          ...
          TypedPtr::U32(raw)=>{
            ...
            let d = data.to_be_bytes();
            let mask_v = masks.to_be_bytes();
            let carry = shift(shift(*self & 0xFF,BYTE_LENGTH as u64-bit, false) as u32, mult(BYTE_LENGTH,4)? as u32, false);
          ...
          TypedPtr::U64(raw)=>{
            ...
            let offset = div(sum(vars.offset,7)?, 8)?;
            let ptr = raw.unc_add(offset)?;
            let carry = shift(shift(*self & 0xFF, BYTE_LENGTH as u64-bit, false) , mult(BYTE_LENGTH,8)? as u64, false);
             let d = [data, carry];
             ...
             shift(t_mask[7] , mult(BYTE_LENGTH, 8)? as u8, false) as u64 & shift(0xFFFFFFFFFFFFFFFF,shft, true) as u64];
        ...
        let last_mask = shift(mask, vars.size as u64-(64-bit), true);
         match ptr_l{
          TypedPtr::U8(raw)=>{
            let ptr = raw.unc_add(vars.offset)?;
            let d_temp = data.to_be_bytes();
            let mask_v_temp = masks.to_be_bytes();                                                                        
            let carry = shift(shift(*self,64-vars.size as u64, true),BYTE_LENGTH as u64-bit, false) as u8;
            let  d = [d_temp[0], d_temp[1], d_temp[2], d_temp[3], d_temp[4], d_temp[5], d_temp[6], d_temp[7], carry];
            let mask_v = [mask_v_temp[0], mask_v_temp[1], mask_v_temp[2], mask_v_temp[3], mask_v_temp[4], mask_v_temp[5], mask_v_temp[6], mask_v_temp[7], shift(last_mask,64-BYTE_LENGTH as u64, true) as u8];
          ...
          TypedPtr::U16(raw)=>{
            ...
            let d = data.to_be_bytes();
            let mask_v = masks.to_be_bytes();
            let carry = shift(shift(shift(*self,64-vars.size as u64, true),BYTE_LENGTH as u64-bit, false) as u16,BYTE_LENGTH as u16, false);
            let m = [u16::from_be_bytes([mask_v[0], mask_v[1]]),u16::from_be_bytes([mask_v[2], mask_v[3]]), 
            u16::from_be_bytes([mask_v[4], mask_v[5]]), u16::from_be_bytes([mask_v[6], mask_v[7]]), shift(last_mask,64-mult(BYTE_LENGTH, 2)? as u64, true) as u16];
            let datas = [u16::from_be_bytes([d[0], d[1]]),u16::from_be_bytes([d[2], d[3]]), 
            u16::from_be_bytes([d[4], d[5]]), u16::from_be_bytes([d[6], d[7]]), carry];
             return return_fn(ptr, new_t_size, &m,&datas, j, slf, vars.overflow, vars.overflow_len, vars.size);
          },
          TypedPtr::U32(raw)=>{
            ...
            let d = data.to_be_bytes();
            let mask_v = masks.to_be_bytes();
            let carry = shift(shift(shift(*self,64-vars.size as u64, true),BYTE_LENGTH as u64-bit, false) as u32, mult(BYTE_LENGTH,4)? as u32, false);
            let m = [u32::from_be_bytes([mask_v[0], mask_v[1],mask_v[2], mask_v[3]]), 
            u32::from_be_bytes([mask_v[4], mask_v[5], mask_v[6],mask_v[7]]), shift(last_mask, 64 - mult(BYTE_LENGTH,4)? as u64, true) as u32];
            let datas = [u32::from_be_bytes([d[0], d[1], d[2], d[3]]), 
            u32::from_be_bytes([d[4], d[5],d[6], d[7]]), carry];
             return return_fn(ptr, new_t_size, &m,&datas, j, slf, vars.overflow, vars.overflow_len, vars.size);
            },
          TypedPtr::U64(raw)=>{
            ...
            let offset = div(sum(vars.offset,7)?, 8)?;
            let ptr = raw.unc_add(offset)?;
            let carry = shift(shift(shift(*self,64-vars.size as u64, true),BYTE_LENGTH as u64-bit, false) , mult(BYTE_LENGTH,8)? as u64, false);
             let d = [data, carry];
             let t_mask = masks.to_be_bytes();
             let mask_v = [u64::from_be_bytes([t_mask[0], t_mask[1], t_mask[2], t_mask[3], t_mask[4], t_mask[5], t_mask[6], t_mask[7]]),
             shift(last_mask,64 - mult(BYTE_LENGTH,8)? as u64, true) ];
             return return_fn(ptr, new_t_size, &mask_v,&d, j, slf, vars.overflow, vars.overflow_len,vars.size);
          }
          _=>{return Err(AllocErr::UnrecognizedInstruction);}
        }
 ...
 impl PushOptimized for usize{
  #[inline]
    fn push_at_buffer(&self, slf:&mut DataB, bit_index:usize, bit_size:&mut Option<usize>, manage_overflow:Option<OverFlow>)->Result<(), AllocErr> {
      match size_of(self){
        2=>{let value = *self as u16; return value.push_at_buffer(slf, bit_index, bit_size, manage_overflow);},
        4=>{let value = *self as u32; return value.push_at_buffer(slf, bit_index, bit_size, manage_overflow);},
        6=>{let value = *self as u64; return value.push_at_buffer(slf, bit_index, bit_size, manage_overflow);},
        _=>{Err(AllocErr::UnrecognizedInstruction)}
      }
    }
 }
 ...
 impl PushOptimized for isize{
  #[inline]
    fn push_at_buffer(&self, slf:&mut DataB, bit_index:usize, bit_size:&mut Option<usize>, manage_overflow:Option<OverFlow>)->Result<(), AllocErr> {
      match size_of(self){
        2=>{let value = *self as i16; return value.push_at_buffer(slf, bit_index, bit_size, manage_overflow);},
        4=>{let value = *self as i32; return value.push_at_buffer(slf, bit_index, bit_size, manage_overflow);},
        6=>{let value = *self as i64; return value.push_at_buffer(slf, bit_index, bit_size, manage_overflow);},
        _=>{Err(AllocErr::UnrecognizedInstruction)}
      }
 ...
 impl PushOptimized for (){
  #[inline(always)]
    fn push_at_buffer(&self, _slf:&mut DataB, _bit_index:usize, _bit_size:&mut Option<usize>, _manage_overflow:Option<OverFlow>)->Result<(), AllocErr> {
      let offset_bit = byte_from_bits(_bit_index, true)?;
      let b = bytes_to_bits(offset_bit.0)?;
      let mut n=false;
      if offset_bit.1.unwrap_or(0) != 0{n=true;}
      upload_ptr(_slf, b, BYTE_LENGTH, n, offset_bit.1.unwrap_or(0))?;
      Ok(())
    }
 }
 ...
 pub struct Vars{
  ...
  #[inline]
  pub fn init<T:SizeOfT+?Sized>(&mut self, slf:&mut DataB, bit_index:usize, bit_size:&mut Option<usize>, _value:&T, manage_overflow:Option<OverFlow>)->Result<(), AllocErr>{
    if check_num(bit_index, usize::MAX){return Err(AllocErr::ArithmeticOverflow);}
    ...
    if self.t_size <=0{
      let b = bytes_to_bits(offset_bit.0)?;
      upload_ptr(slf, b, BYTE_LENGTH, self.no_zero_bit, offset_bit.1.unwrap_or(0))?;
      return Ok(());
    }
            ...
            AllocErr::Overflow(n)=>{
              match n {
                ...
                OverFlowStrategy::RemainingData(_)=>{
                  ...
                  self.t_size = calc_remaining(slf.get_size() , self.offset);
  ...
  #[inline]
  pub fn ret_overflow<T:Copy>(overflow:Option<OverFlowStrategy>, for_remaining_data:Option<usize>, data_vec:&[T], index:usize)->Result<(), AllocErr>
          ...
          OverFlowStrategy::RemainingData(n)=>{
            if let Some(len) = for_remaining_data{
              unsafe{
               ...
               for i in 0..len {
                 *buffer.unc_add(i)? = data_vec[index];
               }
              }
            }else{
  ...
  #[inline]
  pub fn fits<T:Copy>(mut_vars:&mut DataB, data:T, data_size:usize, offset:usize)->Result<(),AllocErr>
  where *mut T: UnconstrainedAdd{
  ...
  #[inline]
  pub fn get_size_of_push(&self)->usize{
    self.t_size
  }
  #[inline]
  pub fn get_bit_size_val(&self)->usize{
    self.size
  }
  #[inline]
  ...
  #[inline]
  pub fn get_overflow_len(&self)->Option<usize>{
    self.overflow_len
  }
  #[inline]
  ...
  #[inline]
  pub fn set_byte_offset(&mut self, new:usize){
    self.offset = new;
  }
  #[inline]
  pub fn set_bit_offset(&mut self, new:usize){
    self.bit = new;
  }
  #[inline]
  ...
  #[inline]
  pub fn set_size_of_push(&mut self, new:usize){
    self.t_size = new;
  }
  #[inline]
  pub fn set_bit_size_val(&mut self, new:usize){
    self.size = new;
  }
  #[inline]
  ...
  #[inline]
  pub fn set_overflow_len(&mut self, new:Option<usize>){
    self.overflow_len = new;
  }
  #[inline]
  pub fn set_remaining_space(&mut self, new:usize){
    self.remaining_space = new;
  }

 }
 #[inline]
 pub fn upload_ptr(slf:&mut DataB, offset_in_bits:usize, byte_len:usize, no_zero_bit:bool, bit_in_byte:usize)->Result<(), AllocErr>{
  if no_zero_bit{
        move_ptr_val(slf, sub(offset_in_bits,byte_len), bit_in_byte)?;
      }
      else{
        move_ptr_val(slf, sub(offset_in_bits,byte_len), 0)?;
      }
   Ok(())
 }
 #[inline]
 pub fn return_fn<T>(ptr:*mut T, ptr_size:usize, masks:&[T], datas:&[T], index:usize, main_structure:&mut DataB, 
  overflow:Option<OverFlowStrategy>, overflow_len:Option<usize>, for_move_ptr:usize)->Result<(), AllocErr>
  where
  T:Copy +
  And<Output = T> +
  Or<Output =T>,
  *mut T: UnconstrainedAdd{
    let j=put_datas(ptr, ptr_size ,&masks, &datas, index )?;
    move_ptr_val(main_structure, main_structure.ptr(), for_move_ptr)?;
    return Vars::ret_overflow(overflow, overflow_len, &datas, j);
 } 
 #[inline]
 pub fn sel_write_strategy<T>(ptr:*mut T,ptr_size:usize, offset:usize, t_size:usize, available_space:usize)->Result<(Option<usize>, BytesSlice<T>), AllocErr>{
 ...
 #[inline]
 pub fn move_ptr_val(ptr:&mut DataB,ptr_value:usize, plus:usize)->Result<(), AllocErr>{
  ptr.move_ptr(sum(ptr_value,plus)?);
  Ok(())
 }
 #[inline]
 pub fn copy_data<T:Copy>(ptr:*mut T, offset:usize, data:T)->Result<(), AllocErr> where *mut T: UnconstrainedAdd{
  unsafe{*ptr.unc_add(offset)? = data;}
  Ok(())
 }
 #[inline]
 pub fn put_datas<T>(ptr:*mut T, ptr_size:usize, masks:&[T], values:&[T], index:usize)->Result<usize, AllocErr>
 where T:And<Output=T> +
 Or<Output = T> +
 Copy, 
 *mut T: UnconstrainedAdd
 {
  let mut j = index;
  let check = sum(index , values.len())?;
  if check > ptr_size {return Err(AllocErr::OutOfBounds(ptr_size));}
  for i in 0..ptr_size{
    unsafe{*ptr.unc_add(i)? = or(and(*(ptr.unc_add(i)?), masks[j]), values[j]);}
    j+=1;
  }
  Ok(j)
 }
 ...
 #[inline]
 pub fn validation_for_push(block_size:usize,byte:usize, bit:Option<usize>, t_size:usize, bit_size:Option<usize>, overflow_f:OverFlow, overflow_len:&mut usize, remaining_space:&mut usize)-> Result<(),AllocErr>{
  // check wether the byte offset is greather than the size of the block data
  if byte > block_size {
        return Err(AllocErr::OutOfBounds(block_size ));
  }
  // Check wether does exist overflow
  if let Some(b) = bit_size{
   if let Some(i) = bit{
    let remaining_bits = calc_remaining(calc_remaining(bytes_to_bits(block_size )?, bytes_to_bits(byte)?), i);
     return manage_overflow(overflow_f, remaining_bits, b, false, overflow_len, remaining_space);
   }
    let t_size_b = bits_to_bytes(b)?;
    let remaining_bytes = calc_remaining(block_size , byte);
   return manage_overflow(overflow_f, remaining_bytes, t_size_b, true, overflow_len, remaining_space);  
  }
  if let Some(i) = bit{
    let b = bytes_to_bits(t_size )?;
    let remaining_bits = calc_remaining(calc_remaining(bytes_to_bits(block_size )?, bytes_to_bits(byte)?), i);
    return manage_overflow(overflow_f, remaining_bits, b, false, overflow_len, remaining_space);
   }
    let remaining_bytes = calc_remaining(block_size , byte);
 ...
 #[inline(always)]
 fn calc_remaining(first:usize, second:usize)->usize{
  sub(first,second)
 }
 #[inline(always)]
 fn check_remaining(remaining:usize, t_size:usize)->bool{
   t_size > remaining
 }
 #[inline]
 fn overflow_strategy(overflow:OverFlow, remaining:usize, t_size:usize, as_byte:bool, overflow_len:&mut usize, remaining_space:&mut usize)->Result<OverFlowStrategy, AllocErr>{
   match overflow{
    OverFlow::LazyFail=>{
 ...
 #[inline]
 fn manage_overflow(overflow:OverFlow, remaining:usize, t_size:usize, as_byte:bool, overflow_len:&mut usize, remaining_space:&mut usize)->Result<(),AllocErr>{
